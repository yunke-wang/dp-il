...
def args_parser():
    ...
    parser.add_argument('--max_step', type=int, default=5000000, help='maximal number of steps. (1 Million default)')
    ...
    parser.add_argument('--noise_type', action="store", default="normal", choices=["normal", "SDNTN", "uniform","salt_pepper"])

    ## diffusion
    parser.add_argument('--action', type=str, default='diff')
    parser.add_argument('--diff_t', type=int, default=10)
    parser.add_argument('--optim_demo', action='store_true')
...
def irl_default_parser(args):
    ...
    if args.c_data == 3:  ## Uniform Noise
        args.noise_level_list = [0.00, 0.25, 0.4, 0.6]
        args.num_opt = 1000
        args.demo_file_size = 1000
        args.worker_num = len(args.noise_level_list)
    
    if args.c_data == 4: ## Salf-and-Pepper Noise
        args.noise_level_list = [0.00, 0.25, 0.4, 0.6]
        args.num_opt = 1000
        args.demo_file_size = 1000
        args.worker_num = len(args.noise_level_list)
...
def iwil_hypers_parser(args):
    if args.env_id == 1:
        score = [0.4, 0.24, 0.12]
    elif args.env_id == 2:
         score = [0., 0., 0.]     
    elif args.env_id == 3:
        score = [0.88, 0.08, 0.08]
    elif args.env_id == 4:
        score = [0.99, 0.3, 0.19]
    else:
        raise NotImplementedError

    args.score = score[3-args.noise_level]
